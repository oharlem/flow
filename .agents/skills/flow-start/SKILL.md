---
description: Use this skill when the user mentions `$flow-start`, asks for `/flow-start`, or describes draft a new change spec.
---

# flow-start

Use this skill when the user mentions `$flow-start`, asks for `/flow-start`, or describes draft a new change spec.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow start`. If the user supplied text after the `$flow-start` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
