---
description: Use this skill when the user mentions `$flow-test`, asks for `/flow-test`, or describes run build verification, tests, and consistency checks.
---

# flow-test

Use this skill when the user mentions `$flow-test`, asks for `/flow-test`, or describes run build verification, tests, and consistency checks.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow test`. If the user supplied text after the `$flow-test` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
