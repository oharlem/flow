---
description: Use this skill when the user mentions `$flow-doctor`, asks for `/flow-doctor`, or describes sanity-check the local Flow installation.
---

# flow-doctor

Use this skill when the user mentions `$flow-doctor`, asks for `/flow-doctor`, or describes sanity-check the local Flow installation.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow doctor`. If the user supplied text after the `$flow-doctor` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
