---
description: Use this skill when the user mentions `$flow-close`, asks for `/flow-close`, or describes close a completed change.
---

# flow-close

Use this skill when the user mentions `$flow-close`, asks for `/flow-close`, or describes close a completed change.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow close`. If the user supplied text after the `$flow-close` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
