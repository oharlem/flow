---
description: Use this skill when the user mentions `$flow-build-task`, asks for `/flow-build-task`, or describes implement one task.
---

# flow-build-task

Use this skill when the user mentions `$flow-build-task`, asks for `/flow-build-task`, or describes implement one task.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow build-task`. If the user supplied text after the `$flow-build-task` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
