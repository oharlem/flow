---
description: Use this skill when the user mentions `$flow-plan`, asks for `/flow-plan`, or describes create an implementation plan and task list.
---

# flow-plan

Use this skill when the user mentions `$flow-plan`, asks for `/flow-plan`, or describes create an implementation plan and task list.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow plan`. If the user supplied text after the `$flow-plan` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
