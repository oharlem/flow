---
description: Use this skill when the user mentions `$flow-amend`, asks for `/flow-amend`, or describes update the active change spec.
---

# flow-amend

Use this skill when the user mentions `$flow-amend`, asks for `/flow-amend`, or describes update the active change spec.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow amend`. If the user supplied text after the `$flow-amend` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
