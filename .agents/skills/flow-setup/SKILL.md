---
description: Use this skill when the user mentions `$flow-setup`, asks for `/flow-setup`, or describes install or upgrade Flow for Codex.
---

# flow-setup

Use this skill when the user mentions `$flow-setup`, asks for `/flow-setup`, or describes install or upgrade Flow for Codex.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow setup`. If the user supplied text after the `$flow-setup` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
