---
description: Use this skill when the user mentions `$flow-run`, asks for `/flow-run`, or describes automate the active roadmap run — all open milestones, or one with `M-N`.
---

# flow-run

Use this skill when the user mentions `$flow-run`, asks for `/flow-run`, or describes automate the active roadmap run — all open milestones, or one with `M-N`.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow run`. If the user supplied text after the `$flow-run` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
