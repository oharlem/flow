---
description: Use this skill when the user mentions `$flow-roadmap`, asks for `/flow-roadmap`, or describes decompose a PRD or notes file into Flow roadmap milestones.
---

# flow-roadmap

Use this skill when the user mentions `$flow-roadmap`, asks for `/flow-roadmap`, or describes decompose a PRD or notes file into Flow roadmap milestones.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow roadmap`. If the user supplied text after the `$flow-roadmap` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
