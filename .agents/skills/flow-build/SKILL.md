---
description: Use this skill when the user mentions `$flow-build`, asks for `/flow-build`, or describes implement all remaining tasks.
---

# flow-build

Use this skill when the user mentions `$flow-build`, asks for `/flow-build`, or describes implement all remaining tasks.

## How to run it

1. Run the canonical Flow command: `FLOW_HOST=codex flow build`. If the user supplied text after the `$flow-build` skill mention, append it after the Flow subcommand. For example, `$flow-start M-2` runs `FLOW_HOST=codex flow start M-2`.
